#!/bin/bash
# Wire ghostty-badges into Devin CLI. Idempotent — safe to re-run.
#
...
# entries for the lifecycle events, replacing any previous ghostty-badge
# entries so upgrades don't duplicate. Coexists with wave-badges entries.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CFG="${DEVIN_CONFIG:-$HOME/.config/devin/config.json}"
BIN="$HOME/.local/bin/ghostty-badge"

command -v jq >/dev/null || { echo "jq is required (brew install jq)"; exit 1; }
[ -f "$CFG" ] || { echo "Devin config not found at $CFG"; exit 1; }

mkdir -p "$HOME/.local/bin"
ln -sf "$SCRIPT_DIR/bin/ghostty-badge" "$BIN"

GB="\"\$HOME/.local/bin/ghostty-badge\" Devin"

jq --arg gb "$GB" '
  def strip: (.hooks //= []) | .hooks |= map(select((.command // "") | test("ghostty-badge") | not));
  def add(state): .hooks += [{"type": "command", "command": ($gb + " " + state)}];
...
echo "ghostty-badges wired into Devin CLI ($CFG). Restart Devin sessions to pick it up."
